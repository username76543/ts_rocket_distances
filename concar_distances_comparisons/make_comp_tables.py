...
def scatter_grid(pairs, titles=None, max_per_fig=9, save_prefix="scatter_page_accs"):
    """
            ...
            for transform_type in all_test_accuracies[test_set].keys():
                ...
                for classifier_type in all_test_accuracies[test_set][transform_type].keys():
                    if classifier_type == classifier_list[i]:
                        x_val = all_test_accuracies[test_set][transform_type][classifier_type]['zero_one']
                    if classifier_type == classifier_list[j]:
                        y_val = all_test_accuracies[test_set][transform_type][classifier_type]['zero_one']
...
for test_set in all_test_times.keys():
    ...
    for transform_type in all_test_times[test_set].keys():
        ...
        all_classifiers[transform_type] = []
        #print(all_test_times[test_set][transform_type].keys())
...
for i in range(0, len(classifier_list)):
    ...
    for j in range(i+1, len(classifier_list)):
        ...
        for test_set in all_test_times.keys():
            ...
            y_val = 0
            for transform_type in all_test_times[test_set].keys():
                #print("here", all_test_times[test_set][transform_type].keys())
                if transform_type == classifier_list[i]:
                    x_val = all_test_times[test_set][transform_type]['data_prepared']
                if transform_type == classifier_list[j]:
                    y_val = all_test_times[test_set][transform_type]['data_prepared']
            print(classifier_list[i], x_val)
            print(classifier_list[j], y_val)
            classifier_acc_compare_xy[classifier_list[i]][classifier_list[j]][0].append(x_val)
            classifier_acc_compare_xy[classifier_list[i]][classifier_list[j]][1].append(y_val)
print(classifier_acc_compare_xy.keys())
x_labels = []
y_labels = []
titles = []
x_y_pairs = []
...
scatter_grid(x_y_pairs, titles, save_prefix="scatter_page_times")
all_classifier_ranks = {}
...
for test_set in all_test_times.keys():
    ...
    for transform_type in all_test_times[test_set].keys():
        #print(all_test_times[test_set])
        element_acc_pairs.append((transform_type, -1*all_test_times[test_set][transform_type]['data_prepared']))
        accuracies.append(-1*all_test_times[test_set][transform_type]['data_prepared'])
    element_acc_pairs = np.array(element_acc_pairs)
    print(accuracies)
    sorted_indices = rank_floats(accuracies)
